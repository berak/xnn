...
struct Layer
{
    ...
    virtual float backward(Volume &upstream, const Volume &downstream, bool training, float globLearn=0) = 0;
...
struct Network
{
    ...
    virtual float backward(Volume &up, const Volume &dn, bool training, float globLearn=0) = 0;
...
Mat viz(const UMat &weights, float fac=1.0f);
